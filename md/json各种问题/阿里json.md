...
json拼接<br>
理论上来讲虽然json是字符串但是和字符串有本质区别 如果将json单纯按照字符串拼接就会出问题
```aidl
        JSONObject jsonObject1 = JSON.parseObject(str);
        JSONObject jsonObjec2 = JSON.parseObject(s1);
        JSONObject jsonObject3 = new JSONObject();
        jsonObject3.putAll(jsonObjec2);
        jsonObject3.putAll(jsonObject1);
        String info = jsonObject3.toString();
```